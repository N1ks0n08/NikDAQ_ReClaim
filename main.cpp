...
int main() {
    {
        ...
        assert(rest_fills.empty()); // Initial resting order, no fills since it's the only order as of now
        ...
        assert(fills.size() == 1); // only 1 resting order, so only 1 fill
        assert(fills[0].price == 100);
        assert(fills[0].aggressive_order_id == 2);
    ...
    std:: cout << "Test case 2 result: PASS\n";
    */
